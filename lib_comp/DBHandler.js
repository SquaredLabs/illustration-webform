...
function start() {
  db.serialize(function () {
    ...
    db.run("CREATE TABLE IF NOT EXISTS requests (wo_number INTEGER PRIMARY KEY AUTOINCREMENT, \
...
function newOrder(data, callback) {
  try {
    ...
    db.run("INSERT INTO requests (wo_number, cover, description, deadline, deadlinedate, \
      ...
      VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0);", data.cover, data.description, data.deadline, data.deadlineDate, data.journalName, data.contactName, data.contactEmail, data.contactPhone, data.KFS);
    db.get("SELECT MAX(wo_number) FROM requests;", (err, row) => callback(err, row["MAX(wo_number)"]));
  } catch (e) {
    console.error(e);
  }
}
...
function getOrders(callback) {
  db.all("SELECT * FROM requests;", (err, row) => callback(err, row));
}
