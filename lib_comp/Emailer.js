...
function dataToHTML(data, confirmation) {
  let confirmationText = confirmation ? `Confirmation for Illustration Request ${data.WorkOrderNumber}` : `Illustration Request ${data.WorkOrderNumber}`;
  return `<html>
...
function sendWOEmail(data, to, confirmation = false) {
  ...
  let subject = confirmation ? `Confirmation for Work Order ${data.WorkOrderNumber}` : `Work Order ${data.WorkOrderNumber}`;
  let message = {
    text: text,
...
function sendEmails(data) {
  try {
    ...
    sendWOEmail(data, `User1 <${process.env.SENDTOEMAIL1}>, User2 <${process.env.SENDTOEMAIL2}>`);
  } catch (e) {
    console.error(e);
  }
}
