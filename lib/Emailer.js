...
function signatureRequestToHTML(data) {
  return `<html>
        <h2>Your Illustration Request has been approved, and needs a signature!</h4>
        <h3><a href="${urlFor('approvalForm/' + data.wo)}">${ urlFor('approvalForm/' + data.wo) }</a></h3>
    </html>`
}
...
function sendSignatureEmail(data, to) {
  let text = signatureRequestToHTML(data)
...
function sendSignatureEmails(data) {
  try {
