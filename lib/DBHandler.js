...
var db = new sqlite3.Database(path.resolve(__dirname,"../Requests.db"))
start()
...
function newOrder(data, callback) {
  try {
    ...
    db.run("INSERT INTO requests (wo_number, cover, description, deadline, deadlinedate, \
      journalname, contactName, contactEmail, contactPhone, KFS) \
      VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, ?, ?);", data.cover,
